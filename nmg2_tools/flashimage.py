...
data format, and facts are not copyrightable. The layout is read from this
project's own specification and from `nmg2_tools.container`, which is this
project's own reader for the same format. No line of any other implementation is copied,
...
decides it. The rule this file obeys is: BUILD THE SEAM, DO NOT BUILD THE
OPTION. So `Cs2ImageBuilder` declares one method,
...
**L2's header format cannot be written until criterion (g) reports it.** Do
not guess it.

WHAT `ContainerLayout` WRITES, AND THE ONE THING IT DOES NOT.

The image is the update container: a 0x14-byte header, a section
...
530 KB on every boot. An image this builder produces
...
# The eight bytes at +0x08 have no known meaning and `nmg2_tools.container`
# does not read them.
...
class ContainerLayout(Cs2ImageBuilder):
    """L1. The image is the update container."""

    def __init__(self, version: int) -> None:
        """Hold the version word the header carries at `+0x00`.

        The plugin state saves the word as a word, so it is carried as it
        was given and never as text.
        """
    ...
    def build(self, sections: Sequence[Cs2Section]) -> bytes:
        ...
        header = bytearray()
        # The longword at +0x04 is not resolved. It is most probably the
        # begin checksum, and `nmg2_tools.container` never verifies it.
