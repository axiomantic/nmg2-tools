...
DSP data: an X workspace, a Y workspace and a P program. The record layout,
read as 32-bit big-endian longwords over a base ``B`` (the pointer-triple
address minus 8):
...
# The CSV column order. The module map's reader takes ``p_ptr`` with ``int(value, 16)``
