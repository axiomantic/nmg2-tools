...
`NMG2_128_OS.bin`; the updater-extraction tests read the real firmware, are
gated on the artifacts, and are the first check that touches a Clavia byte.
...
def build_entry(
    ...
    *,
    ...
    The first 0x20 bytes are the eight named fields. The
...
def test_the_parse_records_the_version_word_and_every_other_header_field():
    """Record the version word. It is saved in plugin state, so the raw
    16-bit value is what is kept."""
...
def test_the_word_at_0x04_is_recorded_and_never_verified():
    """The field is not resolved; it is most probably the begin
...
def test_a_stream_that_produces_more_than_the_declared_length_is_a_failure():
    ...
    This check goes after the decompressor, which is where the m68k loader has
