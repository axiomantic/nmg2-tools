...
cannot claim. A construct a real patch uses that the format specification does
not describe passes T0 and FAILS HERE, and because this tier is informational the
...
# The corpus path is a constant of the artifacts repository's layout:
# `corpus/pch2/` under the artifacts root. This is not a count, and it is not the thing the test must
# not hardcode.
...
def demo_corpus_dir() -> pathlib.Path:
    ...
    The skip discipline: an artifact-gated test that cannot run must skip WITH
    A REASON and must never pass silently. The
    ...
    would be the standard reason spelled a second time, and a message with two
...
def _manifest(corpus: pathlib.Path) -> tuple[int, list[str]]:
    """The corpus manifest: first line is the count, then one row per file."""
...
def test_every_manifest_row_names_an_existing_patch(demo_corpus_dir):
    """The manifest rows are `<path inside the installer><tab><size><tab>
    ...
    The installer directory is NOT asserted here. It is a measurement the
    extractor made, and a test that spelled it would be the hardcoded-figure
    defect this module's header refuses for the count, wearing a path's
    clothes."""
    corpus = demo_corpus_dir
