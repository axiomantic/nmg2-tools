...
| **M2** | The surface answers `read|write` on one line. | T0 | The raw pipe splits this row into five cells. |
| **M3** | The surface answers `read\|write` on one line. | T0 | The escaped pipe keeps this row at four. |

| **M4** | A one-row table states no column count of its own. | T0 | No delimiter row, so no norm. |
