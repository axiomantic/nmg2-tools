...
(2-byte granularity, no fixed stride, the in-range P-pointer test) are the
traps that make the scan hard.
...
# where NMG2_ARTIFACTS is unset, with the gate's reason, via the
# `artifacts_dir` fixture.
# ---------------------------------------------------------------------------
...
def _load_code_image(artifacts_dir):
    ...
    a skip WITH A REASON naming the expected path is required."""
    import os
...
# ---------------------------------------------------------------------------
# The CSV materialisation. The module map declares
# `dsp/g2_module_descriptors.csv` as an input and reads four columns by name, so the header is a contract
...
def test_to_csv_text_renders_every_record_as_the_declared_row():
    ...
    uppercase `0x`-prefixed hex because the module map's reader parses them with
