...
def _family_fixture(family: str):
    ...
    The skip reason is :func:`gated_skip_reason`'s line WORD FOR WORD, prefix
    included. The prefix is carried inside the reason on purpose: pytest's own
    report line reads ``SKIPPED [1] file:line: <reason>``, so without it the
    required literal would never appear in the job output.
