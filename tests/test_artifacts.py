...
# is not a gate. The limit that comes with the pattern: **a skip changes the
# verdict's WORDING and never its exit code.** Scoring it would change what
# `if pytest; then` means for every existing caller, which is a separate
# decision from making the skip visible, and this is only the second of the
# two.
#
...
def test_the_skip_verdict_says_test_in_the_singular_for_one_skip():
    """The noun agrees with the count. A verdict reading `1 tests SKIPPED` is
    a verdict nobody trusts."""
    from nmg2_tools.artifacts import skip_verdict
