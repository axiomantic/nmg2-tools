...
# The first entry starts where the header ends. An entry places the plain
# checksum at +0x10, the compressed length at +0x14, the
...
def test_the_builder_interface_declares_exactly_one_method():
    """The interface declares one method, ``build(sections) -> bytes``. The
    seam for spike criterion (g) is the interface, so its width is the contract. A second
...
def test_container_layout_is_the_one_implementation_today():
    """``ContainerLayout``, which is L1, is the one implementation today. A
    second implementation before spike criterion (g) reports would be a guessed
    L2 header, which is forbidden.
...
def test_the_built_image_parses_to_the_declared_header_and_table():
    """``nmg2_tools.container`` is the reader and this is the writer. The two
    must agree on every field, so the whole parsed object is compared and not one field of it."""
...
def test_two_sections_load_in_table_order_and_the_plain_checksum_verifies():
    """``load_sections`` verifies the checksums, so a wrong checksum raises
...
def test_a_tag_outside_ascii_is_refused():
    """Four characters is not four bytes. The reader decodes the tag as ASCII
    and names the entry when it cannot."""
    with pytest.raises(FlashImageError) as caught:
        ContainerLayout(version=VERSION_1_62).build(
...
def test_every_section_is_stored_because_this_repository_has_no_compressor():
    """``nmg2_tools.lzo1x`` decompresses and nothing in this repository
    compresses, so ``ContainerLayout`` writes the stored form the reader
    already accepts: a compressed length of zero means the bytes at the file
    offset are plain.
