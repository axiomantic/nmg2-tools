...
```bash
python -m pytest tests/test_artifacts.py    # one file
python -m pytest -k <expression>            # by name
```

Narrow by PATH or by `-k`.
...
This is what `.github/workflows/ci.yml` runs, on Python 3.12. That workflow also
asserts the pinned layout before running: a test invoked by a path that does not
hold it is a test that never runs, and collecting zero tests fails the job
rather than passing it.

### Environment

- Requirements: Python 3.11 or later, pytest 7.0 or later. If you work in a
  virtual environment, use its interpreter — a bare `pytest` works only from an
...
- A gated test declares its need by requesting the `artifacts_dir` fixture. A
  fixture NEVER falls back to another root on a miss: a resolver that searched a
  second root would make a fixture's provenance unknowable.
...
from a fork with no configuration. Other tests read a private artifact tree that
`NMG2_ARTIFACTS` locates. When that variable is unset those tests **skip with a
...
**A cross-reference that helps a reader NAVIGATE still stands.** A pointer to
where a related value is also read earns its place and stays, provided it
...
legitimate way to apply it: measure the tree, read every hit in context before
anything is cut, and prove a comment-only sweep changed no behaviour — parse
each modified `.py` file before and after, strip docstrings, and compare
`ast.dump`. Repairing a comment when you change the line it describes is the
other way in, not the only one.
...
- **"ruff clean" in a commit message means nothing without a version.** The same
  unmodified tree reports very different finding counts across ruff releases,
  because later versions turn on far wider default rule sets: a claim that was
  close to true against the tool of its day is false against today's. Do not
  repeat it in a commit message. Cite a run, with the version, instead.
