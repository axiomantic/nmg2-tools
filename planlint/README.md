...
|---|---|---|
| 1 | **A registrar that does not exist at all.** | `registrars_of()` returns an empty list when no task creates the directory's `CMakeLists.txt`. An empty list gives no finding. Delete the one task that creates a test directory's `CMakeLists.txt`, and the lint reports clean. |
