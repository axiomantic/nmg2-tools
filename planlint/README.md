...
| 5 | `payload` | No Clavia-authored content reaches a public repository by ANY route: a `.gitmodules` entry, a `*.pch2` outside the synthesized corpus, a committed fixture above the 65,536-byte ceiling with no allow-listed register row, **or a workflow step that uploads or caches a path intersecting a render, a dump, a capture or a corpus**. **The population is `git ls-files` and the report calls it `tracked files`, which is the name of exactly that set.** It read the DISK and called the result `committed files`: a `.venv` and a `.pytest_cache` were examined, most of what it reported named files that are in no repository, and a reader had no way to tell that report from one taken over the repository. A tree `git` cannot answer for yields nothing and fails as `no-input`; there is no disk fallback, because a fallback would restore the defect silently. | 3.2, 7.8, 22.4 |
...
| 19 | `selfcite` | **Line-number citations into this document, RESOLVED.** A §24.6 row states where a file is named — *"`tests/abi_stub.c` on **4** (lines 4018, 4316, 4694, 9271)"* — and nothing read those numbers. The plan is APPEND-ONLY in normal operation, so every figure below an insertion point is invalidated in silence; `anchors` and `structure` both report clean over it. **Recognition is the document's own citation IDIOM and never a scan for four-digit numbers**: the plan carries a CRC (`0x8926`), sha fragments (`ae9157e`) and line references into OTHER files (`source/MCF5307UM.textlayer.txt:9046`, `(textlayer:9247)`), and a rule that convicted those is a rule an engineer turns off. **A citation must STATE what it claims** — the subject is the nearest preceding backticked path, bound on ONE line — and the claim is then decidable: line N is a `Files:` line naming that path (`line-citation-unresolved`). A citation with no decidable subject is reported as `line-citation-unresolvable` and never passed, because a citation the lint skipped and one it found sound print the same nothing. **A line reference into ANOTHER document is OUT OF SCOPE and the report says so on every run**, clean or dirty; the stated COUNT beside a citation is `counts`'s subject and not this lint's. One unit is one cited line number, which is also the unit a finding names. | 24.6 |
| 20 | `registries` | **A `Check:` target held against the repositories' real test registries — the question a roster cannot answer.** The generated `…impl.check-targets.txt` is derived from the plan by the same extractor `checks` compares it with, so set equality between them can fail for exactly one reason: somebody skipped the generator. Both sides come from one document, and a `Check:` line naming a test nothing registers satisfies it perfectly. This lint reads the OTHER side. `registry-target-unregistered` reports a `ctest -R` name that no `add_test(NAME ...)` in any searched repository registers; `registry-pytest-path-missing` reports a `pytest` invocation whose path exists in none of them; `registry-target-is-placeholder` reports a target the extractor lifted out of PROSE — `pytest <path>`, `pytest tests/…` — which nothing can ever run. **It searches every repository it is given and never attributes a target to one.** §7.1's track table does bind a task prefix to a repository, and `PlanDocument.track_repositories` parses it, but its cells are prose — `gearmulator fork`, `all seven`, `none — a scratch directory` — so a search key would need a hand-maintained translation table beside it, which is the roster defect one level down. "Runnable anywhere" needs no attribution and cannot mislabel. That is also the difference from lint 16's half B, which asks the ATTRIBUTED question and SKIPS when no tree holds the source its task names; `abi_smoke` falls into that skip. **`registered` means named by an `add_test(NAME ...)` in a CMake SOURCE file, read statically — never listed by `ctest`**, so a generated or macro-expanded registration reads as unregistered, and every finding says so. Build trees are excluded, or the answer would depend on who had configured what. **An unreadable tree is never a clean one.** No `--source-repo` skips the lint loudly; a root that is not a directory or a CMake file that cannot be read is `registry-unreadable` and downgrades every absence to `registry-target-unresolved`, because "registered nowhere" and "not searched" demand different repairs; a search that finds zero registrations anywhere is `registry-empty`. The notice names every tree and its contribution on a CLEAN run, so a tree dropped from the search is visible before it matters. A target owned by a task with an anchored `**DONE` is an ERROR and one owned by no complete task is a WARNING; a placeholder is an ERROR either way. One unit is one distinct check target. | 1.3 rule 9; 7.1 |
...
| `repo_registry_good/` | A repository tree registering every check target `clean_plan.md` names, plus the `pytest` path it collects. It exists so that a run giving every conditional flag is CLEAN in every lint: a lint that is always dirty in the all-flags run would make that run useless as the negative case for the SKIPPED notice. |
