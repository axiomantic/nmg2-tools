...
| 6 | `counts` | Every claim the plan states about itself matches what the plan holds: the track rows, the total, the sum, the conditional-task count, and the cross-track edges inside one wave. **The cross-track set is DERIVED from the `Depends:` graph** and held against section 7.3's column in both directions, and assertion 7's number is held against that derived set — the column is an operand and never the source. | 7.3, 7.6 assertions 7 and 13, 24.1, 24.4 |
...
| `neg_counts.md` | A wrong track count, a wrong total, a wrong sum, a wrong conditional count, a wrong cross-track count, an in-wave cross-track edge section 7.3 omits, and a section 7.3 row no `Depends:` line declares. |
